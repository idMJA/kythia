import { Client, GatewayIntentBits, Partials, Options, GuildMember, User } from 'discord.js';
...
export default function kythiaClient(): KythiaClient {
	const client: KythiaClient = new Client({
		intents: [
		...
		sweepers: {
			...
			users: {
				interval: 3600,
				filter: () => (user: User) => user && !user.bot,
			},
		},
	});
	return client;
