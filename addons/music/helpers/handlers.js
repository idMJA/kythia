...
async function handleSeek(interaction, player) {
    let timeInput = interaction.options.getString('time') ?? interaction.options.getInteger('time');
    let seconds = 0;
    ...
    if (isNaN(seconds) || seconds < 0) {
        const embed = new EmbedBuilder()
            .setColor('Red')
            .setDescription('❌ Invalid time format! Please use seconds or mm:ss or hh:mm:ss format.')
            .setFooter(await embedFooter(interaction));
        return interaction.reply({ embeds: [embed] });
    }
    ...
    const embed = new EmbedBuilder()
        .setColor(kythia.bot.color)
        .setDescription(await t(interaction, 'music.helpers.handlers.music.seeked', { time: seconds }))
        .setFooter(await embedFooter(interaction));
    return interaction.reply({ embeds: [embed] });
}
...
async function handleLyrics(interaction, player) {
    ...
    if (!track) {
        const container = new ContainerBuilder().addTextDisplayComponents(
            new TextDisplayBuilder().setContent(await t(interaction, 'music.helpers.handlers.music.lyrics.music.not.found'))
        );
    ...
    if (!lyrics) {
        const container = new ContainerBuilder()
            .setAccentColor(convertColor(kythia.bot.color, { from: 'hex', to: 'decimal' }))
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent(await t(interaction, 'music.helpers.handlers.music.lyrics.lyrics.not.found'))
            );
        return interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
    }
    ...
    if (usedLrclib) {
        ...
        footerText = await t(interaction, 'core.utils.about.embed.footer');
    }
    ...
    const container = new ContainerBuilder()
        .setAccentColor(convertColor(kythia.bot.color, { from: 'hex', to: 'decimal' }))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`## **${embedArtist} - ${embedTitle}**`))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addMediaGalleryComponents(
            new MediaGalleryBuilder().addItems([new MediaGalleryItemBuilder().setURL(track.info.artworkUrl ?? track.info.image ?? null)])
        )
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(trimmedLyrics))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
